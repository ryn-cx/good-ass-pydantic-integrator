...
class TestDiscriminatedUnion:
    ...
    Mirrors the cat/dog test data degenson uses for its own discriminated union
    test.
    """

    def test_single_variant(self) -> None:
        """A single object yields a plain object schema, no ``oneOf``."""
        gapi = GAPI(discriminator_key="type")
        gapi.add_object_from_dict({"type": "cat", "meow": True})
        assert json.loads(gapi.get_json_schema_content()) == {
            "$schema": "http://json-schema.org/schema#",
            "type": "object",
            "properties": {
                "type": {"const": "cat"},
        ...
        lines = gapi.get_pydantic_model_content().splitlines()
        assert "class Cat(BaseModel):" in lines
        assert '    type: Literal["cat"]' in lines
        assert "    meow: bool" in lines

    def test_multiple_variants(self) -> None:
        """Distinct discriminator values become a discriminated ``oneOf``."""
        gapi = GAPI(discriminator_key="type")
        gapi.add_object_from_dict({"type": "cat", "meow": True})
        gapi.add_object_from_dict({"type": "dog", "bark": 5})
        assert json.loads(gapi.get_json_schema_content()) == {
            "$schema": "http://json-schema.org/schema#",
            "oneOf": [
                {
        ...
        content = gapi.get_pydantic_model_content()
        # The root model is a discriminated union keyed on the ``type`` field.
        assert 'Field(..., discriminator="type")' in content
        assert '    type: Literal["cat"]' in content
        assert '    type: Literal["dog"]' in content

    def test_variants_merge_by_discriminator(self) -> None:
        """Objects sharing a discriminator value merge into one variant."""
        gapi = GAPI(discriminator_key="type")
        gapi.add_object_from_dict({"type": "cat", "meow": True})
        gapi.add_object_from_dict({"type": "cat", "meow": False, "lives": 9})
        # A single variant remains (no ``oneOf``); the field only present on the
        # second object becomes optional.
        assert json.loads(gapi.get_json_schema_content()) == {
            "$schema": "http://json-schema.org/schema#",
            "type": "object",
            "properties": {
        ...
        lines = gapi.get_pydantic_model_content().splitlines()
        assert "    meow: bool" in lines
        assert "    lives: int | None = None" in lines

    def test_generated_union_validates_and_routes(self) -> None:
        """The generated union model validates each variant to the right type."""
        gapi = GAPI(discriminator_key="type")
        gapi.add_object_from_dict({"type": "cat", "meow": True})
        gapi.add_object_from_dict({"type": "dog", "bark": 5})
        content = gapi.get_pydantic_model_content()
        ...
        cat = model.model_validate({"type": "cat", "meow": True})
        assert cat.root.type == "cat"
        assert cat.root.meow is True

        dog = model.model_validate({"type": "dog", "bark": 5})
        assert dog.root.type == "dog"
        assert dog.root.bark == 5  # noqa: PLR2004

        # A discriminator value matching no variant fails validation.
        from pydantic import ValidationError  # noqa: PLC0415

        with pytest.raises(ValidationError):
            model.model_validate({"type": "fish", "swim": True})
